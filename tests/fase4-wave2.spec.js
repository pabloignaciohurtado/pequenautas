...
test('Fase 4 · #11: materias nuevas opt-in (enable agrega 3 → 6)', async ({ page }) => {
  await page.goto(fileUrl);
  await page.waitForTimeout(1200);
  const n = await page.evaluate(() => { window.__moreSubjects.enable(); return document.querySelectorAll('.subject:not(.pa53-card):not(.pa54-card):not(.pa55-card):not(.pa58-card)').length; });
  expect(n).toBe(6);
});
