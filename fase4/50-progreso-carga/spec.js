/* Fase 4 #50 - Portón de carga (blocking gate) de recursos.
   ...
   Renumerado de #43 a #50: el número 43 lo reclamaban a la vez esta rama
   y la de los iconos del quiz (hoy #49), así que se separaron. La mascota
   dejó de ser un emoji del sistema (🦉) y ahora es un búho papercraft
   generado con Grok, embebido como data-URI en img/ig-buho.css, en línea
   con la dirección de arte del bosque.
...
(function () {
  ...
  var PA_DONE_KEY = 'pa_precache_done_v5'; // v5 = coincide con CACHE de sw.js
  ...
  function build() {
    if (elWrap || document.getElementById('pa50gate')) return;
    elWrap = document.createElement('div');
    elWrap.id = 'pa50gate';
    elWrap.className = 'pa50gate';
    ...
    elWrap.innerHTML =
      '<div class="pa50card">' +
        '<div class="pa50mascota" aria-hidden="true"></div>' +
        '<div class="pa50title" id="pa50title">Preparando tu bosque…</div>' +
        '<div class="pa50track"><div class="pa50fill" id="pa50fill"></div></div>' +
        '<div class="pa50pct" id="pa50pct">Descargando recursos… 0%</div>' +
        '<div class="pa50hint">Un momento, casi listo para jugar</div>' +
      '</div>';
    (document.body || document.documentElement).appendChild(elWrap);
    elBar = document.getElementById('pa50fill');
    elPct = document.getElementById('pa50pct');
    elTxt = document.getElementById('pa50title');
    // bloquea scroll de fondo mientras el portón está activo
    if (document.documentElement) document.documentElement.classList.add('pa50lock');
  }
  ...
  function dismiss(complete) {
    ...
    if (document.documentElement) document.documentElement.classList.remove('pa50lock');
    if (!elWrap) return;
    elWrap.classList.add('pa50out');
    setTimeout(function () {
