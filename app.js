...
function roundMathCount(){
  ...
  opts.forEach(n=>{ const b=document.createElement('button'); b.className='choice'; b.innerHTML='<span class="cnum">'+n+'</span>'; if(n===count) S.correctBtn=b;
    b.onclick=()=>{ if(n===count){ b.classList.remove('reveal'); b.classList.add('correct'); chime('ok'); const nm=(S.lang==='es'?NUM_ES:NUM_EN)[count]; speakSeq([{t:(S.lang==='es'?('¡Sí! Hay '+nm+'.'):('Yes! There '+(count===1?'is':'are')+' '+nm+'.'))},{t:(S.lang==='es'?'¡Muy bien!':'Great job!')}]); confetti(); afterCorrect('math-'+count); }
...
function roundScience(){
  ...
  ['water','land','sky'].forEach(h=>{ const b=document.createElement('button'); b.className='choice habitat '+h; b.innerHTML='<span class="cface">'+HAB[h].emoji+'</span><span class="clabel">'+HAB[h][S.lang]+'</span>'; if(h===a.hab) S.correctBtn=b;
    b.onclick=()=>{ if(h===a.hab){ b.classList.remove('reveal'); b.classList.add('correct'); chime('ok'); const name=a[S.lang]; const place=HAB[a.hab][S.lang].toLowerCase(); speakSeq([{t:(S.lang==='es'?(name+' vive en '+(a.hab==='sky'?'el cielo':a.hab==='water'?'el agua':'la tierra')+'.'):('Yes! '+name+(a.hab==='land'?' lives on land.':' lives in the '+place+'.')))},{t:(S.lang==='es'?'¡Excelente!':'Well done!')}]); confetti(); afterCorrect('sci-'+a.hab); }
